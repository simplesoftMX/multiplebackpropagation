...
/**
 Method  : void LoadNetworkDialog::OnOK()
 Purpose : Load the network weights.
 Version : 1.0.0
*/
void LoadNetworkDialog::OnOK() {
	CString filename =	filenameBox.GetFileName();

	if (filename.IsEmpty()) {
		WarnUser(_TEXT("You must specify the Back-Propagation Network filename ."), _TEXT("Invalid Network file"));
		filenameBox.SetFocus();
	} else {
		if (parent->LoadNetwork(filenameBox.GetFileName())) LoadSaveDialog::OnOK();
	}
}