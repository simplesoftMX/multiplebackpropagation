...
/**
 Classes  : OutputFileException
 Purpose  : Used to throw Output File exceptions.
...
class OutputFileException : public Exception<OutputFile> {
	public :
		/**
		 Constructor : OutputFileException(CFileException * e, OutputFile * object)
		 Purpose     : Initialise the OutputFileException object.
		 Version     : 1.0.0
		*/
		OutputFileException(CFileException * e, OutputFile * object) : Exception<OutputFile>(object) {
		...
		*/
		void DetermineCauseBased(CFileException * e) {
			cause = _TEXT("Could not write to file « ") + e->m_strFileName + _TEXT(" ».");
			...
			switch(e->m_cause) {
				...
				case CFileException::accessDenied :
					cause += _TEXT(" Access denied. Make sure you have priviledges to write to that file.");
					canRetryOperation = true;
					break;

				case CFileException::directoryFull :
					cause += _TEXT(" Directory full (There are no more directory entries).");
					canRetryOperation = true;
					break;
